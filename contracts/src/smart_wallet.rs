...
impl SmartWallet {
    ...
    pub fn execute_transaction(
        ...
        if transaction.amount > policy.max_amount {
            return Err("Amount exceeds policy limit");
        }
        ...
        {
            return Err("Token not allowed by policy");
        }

        // Verify recipient
        if !policy.allowed_recipients.contains(&transaction.recipient) {
            return Err("Recipient not allowed by policy");
        }
...
mod test {
    ...
    fn test_initialize() {
        ...
        let contract = SmartWallet::new(&e);
    ...
    fn test_set_policy() {
        ...
        let max_amount = 1000;
        let allowed_tokens = vec![&e, symbol!("XLM"), symbol!("USDC")];
        let allowed_recipients = vec![&e, Address::generate(&e)];

        contract.set_policy(&owner, max_amount, allowed_tokens.clone(), allowed_recipients.clone());
    ...
    fn test_execute_transaction() {
        ...
        // Execute transaction
        let result = contract.execute_transaction(&owner, transaction);
        assert!(result.is_ok());
    }
    ...
    fn test_execute_transaction_exceeds_limit() {
        ...
        // Execute transaction
        contract.execute_transaction(&owner, transaction).unwrap();
    }
} 